...
} from "@/lib/claude-code-gateway.js";
import {
  getCodexLoginStatus,
  testCodexConnection,
} from "@/lib/codex-gateway.js";
...
export async function upsertOwnedGatewayResource(input: {
  ...
  const provider = input.provider || "openclaw";
  const isLocalCliProvider = provider === "claude-code" || provider === "codex";

  // For Claude Code, baseUrl and token are not required but we store placeholders
  const baseUrl = isLocalCliProvider
    ? (provider === "codex" ? "local://codex" : "local://claude-code")
    : normalizeGatewayBaseUrl(input.baseUrl);
  const token = isLocalCliProvider
    ? (provider === "codex" ? "codex-cli-auth" : "claude-code-max-plan")
    : input.token.trim();
  const displayName = input.displayName?.trim()
    || (provider === "codex"
      ...
      : isLocalCliProvider
        ? "Claude Code"
        : buildDefaultGatewayDisplayName(baseUrl));

  const existing = isLocalCliProvider
    ? await findMatchingOwnedGatewayByProvider(input.ownerUserId, provider)
...
async function findMatchingOwnedGatewayByProvider(ownerUserId: string, provider: string) {
  const rows = await db
    .select()
    .from(gatewayResources)
    .where(and(eq(gatewayResources.ownerUserId, ownerUserId), eq(gatewayResources.provider, provider)))
    .limit(1);
  return rows[0] ?? null;
}
...
export async function getEffectiveChannelGatewayBinding(channelId: string) {
  const direct = await getChannelGatewayBinding(channelId);
  if (direct) {
    return {
      ...direct,
      inheritedFromChannelId: null as string | null,
    };
  }

  const [channel] = await db
    .select({ parentChannelId: channels.parentChannelId })
    .from(channels)
    .where(eq(channels.id, channelId))
    .limit(1);

  if (!channel?.parentChannelId) return null;

  const parent = await getChannelGatewayBinding(channel.parentChannelId);
  if (!parent) return null;

  return {
    ...parent,
    inheritedFromChannelId: channel.parentChannelId,
  };
}
...
export async function getGatewayRuntimeStateForChannel(
  ...
  const binding = await getEffectiveChannelGatewayBinding(channelId);
  if (!binding) {
  ...
  // Handle local CLI providers separately
  if (binding.resource.provider === "claude-code") {
    try {
      await testClaudeCodeConnection(binding.resource.workspacePath || undefined);
      ...
      const errorMsg = error instanceof Error ? error.message : "Claude Code connection failed";
  ...
  if (binding.resource.provider === "codex") {
    try {
      const login = getCodexLoginStatus();
      if (!login.ok) {
        throw new Error("Codex CLI is not logged in. Run `codex login` first.");
      }
      await testCodexConnection(binding.resource.workspacePath || undefined);
      await persistGatewayValidationState(binding.resource.id, { status: "valid" });
      return {
        ...setGatewayRuntimeState(binding.resource.id, { status: "valid" }),
        gateway: binding,
      ...
      const errorMsg = error instanceof Error ? error.message : "Codex connection failed";
      await persistGatewayValidationState(binding.resource.id, {
        status: "error",
        error: errorMsg,
      });
...
export async function getGatewayRuntimeConfigForChannel(channelId: string) {
  ...
  return {
    ...
    provider: binding.resource.provider || "openclaw",
    workspacePath: binding.resource.workspacePath,
    binding: binding.binding,
    resource: binding.resource,
    inheritedFromChannelId: binding.inheritedFromChannelId,
  };
}

...
class OpenClawGateway {
  constructor() {
    ...
    this._lastTick = null;
    this._lastTickTimeoutWarning = 0;
  ...
  _startTickWatch() {
    ...
    this._tickTimer = setInterval(() => {
      ...
      if (Date.now() - this._lastTick > this._tickIntervalMs * 2) {
        ...
        const now = Date.now();
        if (now - this._lastTickTimeoutWarning > this._tickIntervalMs * 4) {
          console.warn("[OpenClawGW] Tick heartbeat delayed; keeping open WebSocket alive");
          this._lastTickTimeoutWarning = now;
        }
        this._lastTick = now;
      }

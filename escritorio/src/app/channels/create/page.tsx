...
function formatGatewayLabel(gateway: GatewayResourceOption) {
  const label = [gateway.displayName, gateway.baseUrl].filter(Boolean).join(" · ");
  if (gateway.provider && gateway.provider !== "openclaw") {
    return `[Conta conectada] ${label}`;
  }
  return label;
}
...
function CreateChannelPageInner() {
  ...
  const [gatewayProvider, setGatewayProvider] = useState<"openclaw" | "claude-code" | "codex">("openclaw");
  ...
  const openClawStoredGateways = storedGateways.filter((gateway) => !gateway.provider || gateway.provider === "openclaw");
  const selectedStoredGateway = openClawStoredGateways.find((gateway) => gateway.id === selectedGatewayId) ?? null;
  ...
  useEffect(() => {
    ...
    if (openClawStoredGateways.length === 0) {
      if (selectedGatewayId) setSelectedGatewayId("");
      return;
    }
    if (!selectedGatewayId || !openClawStoredGateways.some((gateway) => gateway.id === selectedGatewayId)) {
      setSelectedGatewayId(openClawStoredGateways[0].id);
    }
  }, [gatewayMode, selectedGatewayId, openClawStoredGateways]);
  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      } else if (gatewayMode === "stored") {
        res = await fetch(`/api/gateways/${selectedGatewayId}/test`, { method: "POST" });
      } else {
        res = await fetch("/api/channels/test-gateway", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
            ...
            {gatewayOpen && (
              <div className="p-4 space-y-4 bg-surface/50">
                ...
                {isLocalCliProvider(gatewayProvider) ? (
                  /* ── Claude Code provider UI ── */
                  <div className="space-y-3">
                    <div className="bg-surface-raised rounded-lg p-3 border border-border">
                      <p className="text-xs text-text-secondary mb-2">💡 {gatewayProvider === "codex" ? t("gateway.codexNoApiKey") : t("gateway.claudeCodeNoApiKey")}</p>
                    </div>
                    <div>
                      <label className="block text-xs font-semibold mb-1 text-text-secondary">{t("gateway.workspacePath")}</label>
                      <input
                        type="text"
                        value={workspacePath}
                      ...
                      />
                      <p className="mt-1 text-xs text-text-muted">{t("gateway.workspacePathHelp")}</p>
                    </div>
                  </div>
                ) : (
                  /* ── OpenClaw provider UI ── */
                  <>
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <select
                        ...
                        disabled={storedGatewaysLoading || openClawStoredGateways.length === 0}
                      ...
                      >
                        {openClawStoredGateways.length > 0 ? (
                          <>
                            ...
                            {storedGateways.map((gateway) => (
                              <option
                                key={gateway.id}
                                value={gateway.id}
                                disabled={!!gateway.provider && gateway.provider !== "openclaw"}
                              >
                      ...
                      {openClawStoredGateways.length === 0 && storedGateways.length > 0 && (
                        <p className="mt-1 text-xs text-amber-400">
                          As conexoes de Claude Max/OpenAI continuam salvas, mas nao funcionam como gateway OpenClaw direto.
                        </p>
                      )}

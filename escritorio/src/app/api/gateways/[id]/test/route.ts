...
export async function POST(
  ...
  const provider = accessible.resource.provider || "openclaw";
  if (provider !== "openclaw") {
    return NextResponse.json(
      {
        ok: false,
        agents: [],
        errorCode: "unsupported_saved_gateway_provider",
        error: "This saved connection is not an OpenClaw gateway. Use a real OpenClaw gateway or switch the provider to Claude Code.",
      },
      { status: 400 },
    );
  }

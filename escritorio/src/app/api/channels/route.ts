...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // mapTemplateId is optional if we have a default/fallback
    const effectiveMapTemplateId = (mapTemplateId === "default" || !mapTemplateId) ? null : mapTemplateId;
    ...
    let template;
    if (effectiveMapTemplateId) {
      [template] = await db
        .select()
        .from(mapTemplates)
        .where(eq(mapTemplates.id, effectiveMapTemplateId))
        .limit(1);
    }

    if (!template) {
      // Fallback: pick the first available template
      [template] = await db
        .select()
        .from(mapTemplates)
        .limit(1);
    ...
    if (!template) {
      return NextResponse.json(
        { errorCode: "map_template_not_found", error: "No map templates available in the system" },
        { status: 404 },
      );
    }
    ...
    // Channel type and client metadata
    const channelType = body.channelType === "hq" ? "hq" : body.channelType === "client" ? "client" : "standard";
    const clientName = typeof body.clientName === "string" ? body.clientName.trim().slice(0, 200) || null : null;
    const clientLogo = typeof body.clientLogo === "string" ? body.clientLogo.trim().slice(0, 500) || null : null;
    const parentChannelId = typeof body.parentChannelId === "string" ? body.parentChannelId.trim() || null : null;
    const businessType =
    ...
    const [channel] = await db
      ...
      .values({
        ...
        gatewayConfig: jsonForDb(gatewayConfig ? { taskAutomation: gatewayConfig.taskAutomation || null, libraryToken: crypto.randomUUID() } : { libraryToken: crypto.randomUUID() }),
        channelType,
        clientName,
        clientLogo,
        parentChannelId,
    ...
    if (gatewayConfig?.gatewayId || gatewayConfig?.url || gatewayConfig?.provider === "claude-code" || gatewayConfig?.provider === "codex") {
      try {
        let resource = null;

        if (typeof gatewayConfig.gatewayId === "string" && gatewayConfig.gatewayId) {
          resource = (await getAccessibleGatewayResource(userId, gatewayConfig.gatewayId))?.resource ?? null;
        } else if (gatewayConfig?.provider === "claude-code" || gatewayConfig?.provider === "codex") {
          const provider = gatewayConfig.provider === "codex" ? "codex" : "claude-code";
          resource = await upsertOwnedGatewayResource({
            ownerUserId: userId,
            ...
            provider,
            workspacePath: typeof gatewayConfig.workspacePath === "string" ? gatewayConfig.workspacePath : null,
          });
        } else if (gatewayConfig?.url) {
          resource = await upsertOwnedGatewayResource({
            ownerUserId: userId,
    ...
    if (channelType === "client") {
      try {
        await seedChannelLibrary(
          channel.id,
          getClientLibrarySeed({
            clientName: clientName || name.trim(),
            businessType,
            squads: selectedSquads,
          }),
        );
      } catch (seedErr) {
        console.error("Failed to seed client library during channel creation:", seedErr);
      }
    }

    // --- Default NPC creation ---
    if (defaultNpc && (gatewayConfig?.gatewayId || gatewayConfig?.url || gatewayConfig?.provider === "claude-code" || gatewayConfig?.provider === "codex")) {
      try {

...
export async function GET(
  ...
  const runtimeProvider = req.nextUrl.searchParams.get("provider") || undefined;
  const runtimeModel = req.nextUrl.searchParams.get("model") || undefined;
  ...
  try {
    const gatewayAgents = normalizeGatewayAgents(await internalRpc(id, "agents.list", {
      runtimeProvider,
      runtimeModel,
    }));
    ...
    for (const npc of channelNpcs) {
      ...
      const npcProvider = typeof openclawConfig?.runtimeProvider === "string" ? openclawConfig.runtimeProvider : "openclaw";
      if (agentId && (!runtimeProvider || npcProvider === runtimeProvider)) {
...
export async function DELETE(
  ...
  const inUse = channelNpcs.some((npc) => {
    ...
    const npcProvider = typeof oc?.runtimeProvider === "string" ? oc.runtimeProvider : "openclaw";
    return oc?.agentId === agentId && (!runtimeProvider || npcProvider === runtimeProvider);
  });
  if (inUse) {
    return NextResponse.json(
  ...
  try {
    await internalRpc(id, "agents.delete", {
      agentId,
      deleteFiles: true,
      runtimeProvider,
      runtimeModel,

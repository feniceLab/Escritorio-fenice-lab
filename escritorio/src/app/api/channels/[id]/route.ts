...
import { getEffectiveChannelGatewayBinding } from "@/lib/gateway-resources";
...
export async function GET(
  ...
  try {
    ...
    const gatewayBinding = await getEffectiveChannelGatewayBinding(id);
    ...
    return NextResponse.json({
      channel: {
        ...
        gatewayConfig: {
          ...
          inheritedFromChannelId: gatewayBinding?.inheritedFromChannelId ?? null,
          displayName: gatewayBinding?.resource.displayName ?? null,
          url: gatewayBinding?.resource.baseUrl ?? null,
          provider: gatewayBinding?.resource.provider ?? parsedGatewayConfig?.provider ?? "openclaw",
          workspacePath: gatewayBinding?.resource.workspacePath ?? parsedGatewayConfig?.workspacePath ?? null,
...
export async function PUT(
  ...
  try {
    ...
    if (body.channelType !== undefined && ["hq", "client", "standard"].includes(body.channelType)) {
      updates.channelType = body.channelType;
    }
    if (body.clientName !== undefined) updates.clientName = typeof body.clientName === "string" ? body.clientName.trim().slice(0, 200) || null : null;
    if (body.clientLogo !== undefined) updates.clientLogo = typeof body.clientLogo === "string" ? body.clientLogo.trim().slice(0, 500) || null : null;
    if (body.parentChannelId !== undefined) updates.parentChannelId = typeof body.parentChannelId === "string" ? body.parentChannelId.trim() || null : null;

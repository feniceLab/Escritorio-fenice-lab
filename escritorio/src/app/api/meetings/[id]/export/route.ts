...
function formatMinutesMarkdown(m: {
  ...
  const normalizedRecord = normalizeMeetingMinutesRecord(m);
  const participants = normalizedRecord.participants as { name: string }[];
  const keyTopics = normalizedRecord.keyTopics as string[];
  ...
  if (normalizedRecord.conclusions) {
    md += `## ${translateServer(normalizedLocale, "meeting.conclusions")}\n${normalizedRecord.conclusions}\n\n`;
  }

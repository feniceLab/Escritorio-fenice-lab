...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (body.positionX !== undefined || body.positionY !== undefined) {
      const nextPositionX = Number(body.positionX);
      const nextPositionY = Number(body.positionY);
      if (!Number.isInteger(nextPositionX) || !Number.isInteger(nextPositionY) || nextPositionX < 0 || nextPositionY < 0) {
        return NextResponse.json({ errorCode: "invalid_npc_position", error: "Invalid NPC position" }, { status: 400 });
      }

      const [occupiedNpc] = await db.select({ id: npcs.id })
        .from(npcs)
        .where(and(
          eq(npcs.channelId, npc.channelId),
          eq(npcs.positionX, nextPositionX),
          eq(npcs.positionY, nextPositionY),
          ne(npcs.id, id),
        ))
        .limit(1);

      if (occupiedNpc) {
        return NextResponse.json({ errorCode: "npc_position_occupied", error: "That tile is already occupied by another NPC" }, { status: 409 });
      }

      updates.positionX = nextPositionX;
      updates.positionY = nextPositionY;
    }
    ...
    if (body.passPolicy !== undefined) {
      ...
      if (existingAgentId && body.passPolicy?.trim()) {
        try {
          ...
          const sectionHeader = "## Guia de comportamento em reunião";
          const newSection = `${sectionHeader}\n${body.passPolicy.trim()}`;
          const legacyMeetingGuidePattern = /## (?:(?:Guia de comportamento em reunião)|(?:[^\n]*[\u3131-\uD79D][^\n]*))[\s\S]*?(?=\n## |$)/;
          const updated = legacyMeetingGuidePattern.test(identityContent)
            ? identityContent.replace(legacyMeetingGuidePattern, newSection)
            : identityContent + "\n\n" + newSection;

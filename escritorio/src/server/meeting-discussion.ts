...
import { MEETING_NPC_STREAM_EVENT } from "./meeting-socket.ts";
...
type MeetingGatewayRuntime = {
  chatAbort?: (agentId: string, sessionKey: string) => Promise<void> | void;
};
...
type RegisterMeetingDiscussionHandlersArgs = {
  ...
  deps: {
    ...
    user: MeetingUser;
    getOrConnectGateway: (channelId: string, overrides?: {
      provider?: string | null;
      workspacePath?: string | null;
      model?: string | null;
    }) => Promise<MeetingGatewayRuntime | null>;
    ...
    sendNpcMessageViaRuntime: (input: {
      channelId: string;
      npcConfig: MeetingNpcConfig;
      sessionKey: string;
      message: string;
...
export function registerMeetingDiscussionHandlers({
  ...
  socket.on("meeting:start-discussion", async (payload: unknown) => {
    ...
    const existing = activeBrokers.get(channelId);
    if (existing) {
      if (existing.isRunning()) {
        socket.emit("meeting:error", { error: "A meeting is already in progress" });
        return;
      }
      // Stale broker (crashed / stuck) — clean up and allow restart.
      console.warn(`[meeting] Cleaning up stale broker for channel ${channelId}`);
    ...
    const needsChannelDefaultGateway = aiNpcs.some((npc) => !npc.runtimeProvider || npc.runtimeProvider === "openclaw");
    if (needsChannelDefaultGateway) {
    ...
    const npcByAgentId = new Map<string, MeetingNpcConfig>(
      aiNpcs
        .filter((npc): npc is MeetingNpcConfig & { agentId: string } => !!npc.agentId)
        .map((npc) => [npc.agentId, npc]),
    );
    const meetingGatewayProxy = {
      chatSend: async (
        agentId: string,
        sessionKey: string,
        message: string,
        ...
        const npcConfig = npcByAgentId.get(agentId);
        if (!npcConfig?.agentId) {
          throw new Error(`No runtime configuration found for meeting agent ${agentId}`);
        }
        const result = await sendNpcMessageViaRuntime({
          channelId,
          npcConfig,
          sessionKey,
          message,
          onDelta: onChunk,
          logPrefix: "meeting-broker",
        });
    ...
    const brokerInstance = createMeetingBroker(
      {
        topic,
        participants,
        gateway: meetingGatewayProxy,
        sessionKeyPrefix: channelId.slice(0, 8),
        meetingId,
        settings: settings || {},
        quota: {
      ...
      },
      {
        ...
        onTurnStart: (agent) => {
          const npcConfig = npcByAgentId.get(agent.agentId);
          io.to(getMeetingRoomId(channelId)).emit("meeting:npc-turn-start", {
            npcId: npcConfig?.id || agent.npcId || agent.agentId,
        ...
        onTurnEnd: (agentId, fullResponse) => {
          const npcConfig = npcByAgentId.get(agentId);
          const npcId = npcConfig?.id || agentId;
          io.to(getMeetingRoomId(channelId)).emit(MEETING_NPC_STREAM_EVENT, {
            npcId,
            agentId,
            npcName: npcConfig?.name || participants.find((participant) => participant.agentId === agentId)?.displayName || agentId,
            chunk: "",
            done: true,
          });
          ...
          void (globalThis as any).telegramManager?.sendMeetingMessage?.({
            channelId,
            sender: agent?.displayName || agentId,
            content: fullResponse,
            senderType: "npc",
        ...
        onMeetingEnd: async (transcript, durationSeconds) => {
          ...
          void (globalThis as any).telegramManager?.sendMeetingMessage?.({
            channelId,
            sender: "Sistema",
            content: `Reuniao encerrada. Ata: ${minutesId || "nao gerada"}. ${summary.conclusions || ""}`.trim(),
            senderType: "system",
          });
  ...
  socket.on("meeting:direct-speak", async (payload: unknown) => {
    ...
    const agent = broker.config.participants.find((participant) => (
      participant.npcId === npcId || participant.agentId === npcId
    ));
    if (!agent?.agentId) {
    ...
    broker.directSpeak(agent.agentId);
  });

...
function ensureSqliteCompatibility(sqlite) {
  ...
  applySqliteAlterStatements(sqlite, "characters", [
    "ALTER TABLE characters ADD COLUMN telegram_chat_id TEXT",
  ]);
...
if (isPostgres) {
  ...
  const gatewayResources = pgTable("gateway_resources", {
    ...
    provider: varchar("provider", { length: 40 }).default("openclaw").notNull(),
  ...
  const npcMemoryItems = pgTable("npc_memory_items", {
    id: uuid("id").primaryKey().defaultRandom(),
  ...
  const telegramClientGroups = pgTable("telegram_client_groups", {    id: uuid("id").defaultRandom().primaryKey(),    clientId: uuid("client_id"),    channelId: uuid("channel_id"),    clientSlug: varchar("client_slug", { length: 100 }),    clientName: varchar("client_name", { length: 200 }).notNull(),    telegramChatId: varchar("telegram_chat_id", { length: 50 }).notNull().unique(),    telegramWebUrl: text("telegram_web_url"),    groupType: varchar("group_type", { length: 30 }).notNull().default("client"),    allowedNpcs: jsonb("allowed_npcs").notNull().default([]),    notificationRules: jsonb("notification_rules").notNull().default({}),    active: boolean("active").notNull().default(true),    createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),    updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),  });  const telegramAgentRoutines = pgTable("telegram_agent_routines", {    id: uuid("id").defaultRandom().primaryKey(),    routineSlug: varchar("routine_slug", { length: 100 }).notNull().unique(),    npcId: uuid("npc_id"),    label: varchar("label", { length: 200 }).notNull(),    scheduleLocal: varchar("schedule_local", { length: 50 }).notNull(),    timezone: varchar("timezone", { length: 60 }).notNull().default("America/Sao_Paulo"),    targetScope: varchar("target_scope", { length: 50 }).notNull().default("operational_group"),    actionType: varchar("action_type", { length: 80 }).notNull().default("telegram_digest"),    payloadJson: jsonb("payload_json").notNull().default({}),    active: boolean("active").notNull().default(true),    lastRunKey: varchar("last_run_key", { length: 100 }),    lastRunAt: timestamp("last_run_at", { withTimezone: true }),    createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),    updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),  });
  schema = {
    users,
    characters,
    groups,
    ...
    tasks,
    npcReports,
    telegramClientGroups,
    telegramAgentRoutines,
  };
...
// ─── SQLite mode ──────────────────────────────────────────────────────────────
  ...
  const gatewayResources = sqliteTable("gateway_resources", {
    ...
    tokenEncrypted: text("token_encrypted").notNull(),
    pairedDeviceId: text("paired_device_id"),
    provider: text("provider").default("openclaw").notNull(),
    workspacePath: text("workspace_path"),
  ...
  const npcMemoryItems = sqliteTable("npc_memory_items", {
    id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),
  ...
  const telegramClientGroups = sqliteTable("telegram_client_groups", {
    id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),
  ...
  const telegramAgentRoutines = sqliteTable("telegram_agent_routines", {
    id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),

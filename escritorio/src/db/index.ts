...
export const automationJobs = activeSchema.automationJobs;
export const approvalRequests = activeSchema.approvalRequests;
export const npcActionLogs = activeSchema.npcActionLogs;
...
export const mcpServers = activeSchema.mcpServers;
export const skills = activeSchema.skills;
export const channelMcpServers = activeSchema.channelMcpServers;
export const channelSkills = activeSchema.channelSkills;
export const channelLibraryItems = activeSchema.channelLibraryItems;
export const npcLibraryItems = activeSchema.npcLibraryItems;
export const npcMemoryItems = activeSchema.npcMemoryItems;
export const agencyReports = activeSchema.agencyReports;
export const tokenUsageLogs = activeSchema.tokenUsageLogs;
export const officeClients = activeSchema.officeClients;
export const officeMemories = activeSchema.officeMemories;
export const officeNotifications = activeSchema.officeNotifications;
export const officeContextSnapshots = activeSchema.officeContextSnapshots;
export const officeAgentRuns = activeSchema.officeAgentRuns;
...
export function ensureSqliteCompatibility(sqlite: BetterSqlite3.Database) {
  ...
  sqlite.exec(`
    CREATE TABLE IF NOT EXISTS channel_library_items (
      id TEXT PRIMARY KEY NOT NULL,
      channel_id TEXT NOT NULL REFERENCES channels(id) ON DELETE CASCADE,
      layer TEXT NOT NULL,
    ...
    CREATE INDEX IF NOT EXISTS idx_channel_library_items_channel_layer ON channel_library_items(channel_id, layer);
    CREATE UNIQUE INDEX IF NOT EXISTS channel_library_items_unique ON channel_library_items(channel_id, layer, category, name);
    CREATE TABLE IF NOT EXISTS npc_library_items (
      id TEXT PRIMARY KEY NOT NULL,
      npc_id TEXT NOT NULL REFERENCES npcs(id) ON DELETE CASCADE,
      layer TEXT NOT NULL,
      category TEXT NOT NULL,
    ...
    CREATE INDEX IF NOT EXISTS idx_npc_library_items_npc_layer ON npc_library_items(npc_id, layer);
    CREATE UNIQUE INDEX IF NOT EXISTS npc_library_items_unique ON npc_library_items(npc_id, layer, category, name);
    CREATE TABLE IF NOT EXISTS npc_memory_items (
      id TEXT PRIMARY KEY NOT NULL,
      npc_id TEXT NOT NULL REFERENCES npcs(id) ON DELETE CASCADE,
    ...
    CREATE INDEX IF NOT EXISTS idx_npc_memory_items_npc_type ON npc_memory_items(npc_id, memory_type);
    CREATE INDEX IF NOT EXISTS idx_npc_memory_items_npc_created ON npc_memory_items(npc_id, created_at);
    ...
    CREATE INDEX IF NOT EXISTS idx_token_usage_npc ON token_usage_logs(npc_id);
  ...
  applySqliteAlterStatements(sqlite, "channels", [
    ...
    "ALTER TABLE channels ADD COLUMN channel_type TEXT DEFAULT 'standard'",
    "ALTER TABLE channels ADD COLUMN client_name TEXT",
    "ALTER TABLE channels ADD COLUMN client_logo TEXT",
    "ALTER TABLE channels ADD COLUMN parent_channel_id TEXT",
  ]);
  applySqliteAlterStatements(sqlite, "npcs", [
    "ALTER TABLE npcs ADD COLUMN total_tokens INTEGER NOT NULL DEFAULT 0",
    "ALTER TABLE npcs ADD COLUMN reports_to_id TEXT",
  ]);
  sqlite.exec("CREATE INDEX IF NOT EXISTS idx_npcs_reports_to_id ON npcs(reports_to_id)");
  applySqliteAlterStatements(sqlite, "gateway_resources", [
    "ALTER TABLE gateway_resources ADD COLUMN provider TEXT NOT NULL DEFAULT 'openclaw'",
    "ALTER TABLE gateway_resources ADD COLUMN workspace_path TEXT",
  ]);
  applySqliteAlterStatements(sqlite, "token_usage_logs", [
    "ALTER TABLE token_usage_logs ADD COLUMN provider TEXT",
    "ALTER TABLE token_usage_logs ADD COLUMN tool_name TEXT",
    "ALTER TABLE token_usage_logs ADD COLUMN operation_id TEXT",
    "ALTER TABLE token_usage_logs ADD COLUMN duration_ms INTEGER",
    "ALTER TABLE token_usage_logs ADD COLUMN label TEXT",
    "ALTER TABLE token_usage_logs ADD COLUMN is_real_usage INTEGER NOT NULL DEFAULT 0",
  ]);
  sqlite.exec("CREATE INDEX IF NOT EXISTS idx_token_usage_kind ON token_usage_logs(context_kind)");
  applySqliteAlterStatements(sqlite, "tasks", [
    ...
    "ALTER TABLE tasks ADD COLUMN assigner_npc_id TEXT",
    "ALTER TABLE tasks ADD COLUMN recurrence TEXT NOT NULL DEFAULT 'once'",
    "ALTER TABLE tasks ADD COLUMN scheduled_time TEXT",
    "ALTER TABLE tasks ADD COLUMN scheduled_day INTEGER",
  ...
  sqlite.exec("CREATE INDEX IF NOT EXISTS idx_tasks_assigner_npc ON tasks(assigner_npc_id)");
  sqlite.exec("CREATE INDEX IF NOT EXISTS idx_tasks_status_approval ON tasks(status, requires_approval)");


  // ── MCP Servers, Skills & Agency Reports ──────────────────────────────
  sqlite.exec(`
    ...
    CREATE TABLE IF NOT EXISTS channel_mcp_servers (
      id TEXT PRIMARY KEY NOT NULL,
      channel_id TEXT NOT NULL REFERENCES channels(id) ON DELETE CASCADE,
      mcp_server_id TEXT NOT NULL REFERENCES mcp_servers(id) ON DELETE CASCADE,
      created_at TEXT
    );
    CREATE UNIQUE INDEX IF NOT EXISTS channel_mcp_servers_unique
      ON channel_mcp_servers(channel_id, mcp_server_id);
    ...
    CREATE UNIQUE INDEX IF NOT EXISTS channel_skills_unique
      ON channel_skills(channel_id, skill_id);
    CREATE TABLE IF NOT EXISTS agency_reports (
      id TEXT PRIMARY KEY NOT NULL,
      channel_id TEXT NOT NULL REFERENCES channels(id) ON DELETE CASCADE,
      agent_id TEXT,
      content TEXT NOT NULL,
      analysis_mode TEXT DEFAULT 'daily',
      report_date TEXT,
      created_at TEXT
    );
    CREATE INDEX IF NOT EXISTS idx_agency_reports_channel
      ON agency_reports(channel_id);
  `);

...
interface NpcCallToPlayerPayload {
  npcId: string;
  message?: string;
  reportId?: string;
  reportKind?: string;
...
class NpcSprite {
  ...
  constructor(scene: Phaser.Scene, data: NpcData) {
    ...
    if (!scene.sys?.displayList) return;
  ...
  private applyTexture(textureKey: string, dataUrl: string): void {
    ...
    img.onload = () => {
      try {
        if (!this.scene?.sys?.displayList || !this.scene.textures) return;
        // Use a unique key to avoid texture removal race conditions
        const uniqueKey = `${textureKey}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  ...
  updateFromData(data: { name?: string; direction?: string; appearance?: unknown; positionX?: number; positionY?: number }): void {
    ...
    if (typeof data.positionX === "number" && typeof data.positionY === "number") {
      this.homeCol = data.positionX;
      this.homeRow = data.positionY;
      this.pixelX = data.positionX * TILE_SIZE + TILE_SIZE / 2;
      this.pixelY = data.positionY * TILE_SIZE + TILE_SIZE / 2;
      ...
      this.stopWalkAnimation();
      if (this.highlightGlow && this.isHighlighted) {
...
export class GameScene extends Phaser.Scene {
  ...
  // Path following
  private playerTextureLoaded = false;
  ...
  create(): void {
    ...
    if (initialChannelData) {
      ...
      // Load initial NPCs passed from React
      if (initialChannelData.npcs) {
        this.loadNpcs(initialChannelData.npcs);
      }
    ...
    if (tiledJsonData) {
      ...
      // No channel map available — use built-in small-office template so the
      // room renders with walls and furniture instead of an empty beige floor.
      this.loadTiledMap(smallOfficeTemplate.tiledJson as unknown as Record<string, unknown>);
    }
    ...
    if (this.input.keyboard) {
      ...
      const onFocusIn = (e: FocusEvent) => {
        if (!this.sys?.isActive() || !this.input?.keyboard) return;
        if (shouldReleaseKeyboardCapture(e.target)) {
          this.input.keyboard.enabled = false;
          this.input.keyboard.removeCapture(capturedKeys);
        }
      };
      const onFocusOut = () => {
        if (!this.sys?.isActive() || !this.input?.keyboard) return;
        ...
        this.time.delayedCall(50, () => {
          if (!this.sys?.isActive() || !this.input?.keyboard) return;
          if (shouldReleaseKeyboardCapture(document.activeElement)) return;
          this.input.keyboard.addCapture(capturedKeys);
    ...
    this.input.keyboard?.on("keydown-ESC", () => {
      ...
      if (this.spawnSetMode) { EventBus.emit("spawn-cancel"); }
    });
    ...
    // DEPRECATED: We now trust React's NPC list via initialChannelData and npc:sync-all
    /*
    if (this.channelId) {
    ...
    */
    ...
    EventBus.on("spritesheet-ready", (dataUrl: string) => {
      if (this.playerTextureLoaded) return;
    ...
    const gameSceneListeners = {
      "dialog:open": () => { this.dialogOpen = true; },
      "dialog:close": () => { this.dialogOpen = false; },
      "placement-mode-start": () => { this.placementMode = true; },
      "placement-mode-end": () => {
        this.placementMode = false;
        this.placementHighlight?.destroy();
        this.placementHighlight = null;
      },
      "spawn-set-mode-start": () => { this.spawnSetMode = true; },
      "spawn-set-mode-end": () => {
        this.spawnSetMode = false;
        this.spawnHighlight?.destroy();
        this.spawnHighlight = null;
      },
      "task-automation-updated": (data: unknown) => {
        if (data && typeof data === "object" && typeof (data as { reportWaitSeconds?: unknown }).reportWaitSeconds === "number") {
          const { reportWaitSeconds } = data as { reportWaitSeconds: number };
          this.reportWaitMs = Math.max(5000, reportWaitSeconds * 1000);
        }
      },
      "owner-status": (data: unknown) => {
        if (data && typeof data === "object" && typeof (data as { isOwner?: unknown }).isOwner === "boolean") {
          this.isChannelOwner = (data as { isOwner: boolean }).isOwner;
        }
      },
      "npc:spawn-local": (raw: { id: string; name: string; positionX: number; positionY: number; direction?: string; appearance?: unknown }) => {
        const npcData: NpcData = { ...raw, direction: raw.direction || "down" };
        if (this.npcSprites.some(n => n.id === npcData.id)) return;
        const npc = new NpcSprite(this, npcData);
        this.npcSprites.push(npc);
      ...
      "npc:remove-local": (data: { npcId: string }) => { this.removeNpcById(data.npcId); },
      "npc:update-local": (data: NpcUpdatePayload) => {
        const npc = this.npcSprites.find(n => n.id === data.npcId);
        if (!npc) return;
        const previousCol = npc.homeCol;
        const previousRow = npc.homeRow;
        npc.updateFromData(data);
        if (typeof data.positionX === "number" && typeof data.positionY === "number") {
          this.npcTilePositions.delete(`${previousCol},${previousRow}`);
          this.npcTilePositions.add(`${data.positionX},${data.positionY}`);
        }
      },
      "npc:start-move": (data: { npcId: string; targetCol: number; targetRow: number; message?: string }) => {
        const npc = this.npcSprites.find(n => n.id === data.npcId);
        if (!npc || npc.moveState !== "idle") return;
        this.npcTilePositions.delete(`${npc.homeCol},${npc.homeRow}`);
        npc.moveTo(
      ...
      "npc:sync-all": (npcs: NpcData[]) => {
        if (!this.sys?.displayList) return;
        this.npcSprites.forEach(npc => npc.destroy());
        this.npcSprites = [];
        this.npcTilePositions.clear();
        this.loadNpcs(npcs);
      },
      "npc:call-to-player": (data: NpcCallToPlayerPayload) => {
        if (!this.player) return;
        const playerCol = Math.floor(this.player.x / TILE_SIZE);
        const playerRow = Math.floor(this.player.y / TILE_SIZE);
        const npc = this.npcSprites.find(n => n.id === data.npcId);
        ...
        if (dist < TILE_SIZE + 4) {
          ...
          npc.waitTimer = 0;
          EventBus.emit("npc:bubble", { npcId: npc.id, text: npc.arrivalBubbleText || undefined });
          EventBus.emit("toast:show", { message: `Press / to talk to ${data.npcName || npc.name}` });
          EventBus.emit("npc:movement-arrived", {
            npcId: npc.id, npcName: data.npcName || npc.name,
            pendingMessage: npc.pendingMessage, reportId: npc.pendingReportId, reportKind: npc.pendingReportKind,
          });
          return;
        }
        this.npcTilePositions.delete(`${npc.homeCol},${npc.homeRow}`);
        npc.moveTo(playerCol, playerRow, findPath, this.createNpcWalkValidator(npc, playerCol, playerRow), {
          message: data.message, reportId: data.reportId, reportKind: data.reportKind,
          bubbleText: data.bubbleText, waitDurationMs: data.reportKind === "complete" ? this.reportWaitMs : 10000,
        });
      },
      "npc:deliver-response": (data: { npcId: string; npcName: string }) => {
        if (!this.player) return;
        const npc = this.npcSprites.find(n => n.id === data.npcId);
        if (!npc) return;
        const dist = npc.distanceTo(this.player.x, this.player.y);
      ...
      "npc:start-return": (data: { npcId: string }) => {
        const npc = this.npcSprites.find(n => n.id === data.npcId);
        if (!npc || npc.moveState !== "waiting") return;
        npc.returnToHome(findPath, (tx: number, ty: number) => this.isWalkable(tx, ty) && !this.isTileOccupied(tx, ty));
      },
      "npc:approach-and-interact": (data: { npcId: string; npcName?: string }) => {
        this.approachNpcAndInteract(data.npcId, data.npcName);
      },
      "spritesheet-ready": (dataUrl: string) => {
        if (this.playerTextureLoaded) return;
        this.playerTextureLoaded = true;
        this.loadPlayerTexture(dataUrl);
      },
      "socket-ready": handleSocketReady,
      "chat:bubble": (data: { senderId: string }) => { this.showPlayerBubble(data.senderId); },
      "npc:bubble": (data: { npcId: string; text?: string }) => { this.showNpcBubbleIcon(data.npcId, data.text); },
      "npc:bubble-clear": (data: { npcId: string }) => { this.clearNpcBubble(data.npcId); },
      "request-player-position": () => {
        if (this.player) EventBus.emit("player-position-response", { x: this.player.x, y: this.player.y });
      },
    };

    // Register all listeners
    for (const [ev, fn] of Object.entries(gameSceneListeners)) {
      EventBus.on(ev, fn);
    }

    // Cleanup specifically these scene listeners on shutdown
    this.events.once(Phaser.Scenes.Events.SHUTDOWN, () => {
      for (const [ev, fn] of Object.entries(gameSceneListeners)) {
        EventBus.off(ev, fn);
      }
  ...
  private setupSocketListeners(): void {
    ...
    this.socket.on("npc:updated", (data: { npcId: string; name?: string; direction?: string; appearance?: unknown; positionX?: number; positionY?: number }) => {
      const npc = this.npcSprites.find(n => n.id === data.npcId);
      if (!npc) return;
      const previousCol = npc.homeCol;
      const previousRow = npc.homeRow;
  ...
  private showNpcBubbleIcon(npcId: string, text?: string): void {
    if (!this.sys?.displayList || !this.add) return;

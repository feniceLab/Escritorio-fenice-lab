...
export function MapCanvas({ state, dispatch, findTileset, onStatusUpdate, layerOverlayMap, onEditSelectionPixels, onCopySelection, onSaveAsStamp, activeStamp, onPlaceStamp }: MapCanvasProps) {
  ...
  const applyTool = useCallback(
    (mx: number, my: number) => {
      // [MapDebug] Logs temporários — remover depois
      console.log('[MapDebug] applyTool called', {
        mx, my,
        tool: state.tool,
      ...
      if (!state.mapData) { console.warn('[MapDebug] BLOCKED: no mapData'); return; }
      const layer = state.mapData.layers[state.activeLayerIndex];
      console.log('[MapDebug] active layer:', { name: layer?.name, type: layer?.type, hasData: !!layer?.data, visible: layer?.visible });
      if (!layer || layer.type !== 'tilelayer' || !layer.data) {
        console.warn('[MapDebug] BLOCKED: layer not a tilelayer or no data', { layerType: layer?.type });
        return;
      }
      ...
      console.log('[MapDebug] tile coord:', tile, 'mapSize:', { mapW, mapH });

      if (tile.x < 0 || tile.x >= mapW || tile.y < 0 || tile.y >= mapH) {
        console.warn('[MapDebug] BLOCKED: tile out of bounds');
        return;
      }
      ...
      if (state.tool === 'erase') {
        ...
        if (region && (region.width > 1 || region.height > 1)) {
          ...
          console.log('[MapDebug] paint single tile:', { idx, oldGid, newGid, willPaint: oldGid !== newGid });
          if (newGid === 0) {
            console.warn('[MapDebug] ⚠️ selectedTileGid is 0 — click a color in the palette first!');
          }
          if (oldGid !== newGid) {
            changes.push({ index: idx, oldGid, newGid });
          }
      ...
      console.log('[MapDebug] changes to dispatch:', changes.length);
      if (changes.length > 0) {
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      console.log('[MapDebug] handleMouseDown', {
        button: e.button,
        tool: state.tool,
        hasMapData: !!state.mapData,
      ...
      const canvas = canvasRef.current;
      if (!canvas || !state.mapData) {
        console.warn('[MapDebug] handleMouseDown BLOCKED: no canvas or mapData');
        return;
      }
      ...
      // 1. Pan check
      if (panZoom.handleMouseDown(e, rect)) { console.log('[MapDebug] consumed by pan'); return; }

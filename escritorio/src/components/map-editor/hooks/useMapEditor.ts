...
function resizeTileLayerData(data: number[], oldWidth: number, oldHeight: number, newWidth: number, newHeight: number): number[] {
  ...
  for (let row = 0; row < copyHeight; row++) {
    for (let col = 0; col < copyWidth; col++) {
      resized[row * newWidth + col] = data[row * oldWidth + col];
    }
  }
...
function clampObjectPosition(value: number, tileSize: number, maxTiles: number): number {
  const maxPosition = Math.max(0, (maxTiles - 1) * tileSize);
  return Math.min(Math.max(value, 0), maxPosition);
}

function resizeLayer(layer: TiledLayer, mapData: TiledMap, width: number, height: number): TiledLayer {
  if (layer.type === 'tilelayer' && layer.data) {
    return {
      ...layer,
      width,
      height,
      data: resizeTileLayerData(layer.data, mapData.width, mapData.height, width, height),
    };
  }
  ...
  if (layer.type === 'objectgroup' && layer.objects) {
    return {
      ...
      objects: layer.objects.map((object) => ({
        ...object,
        x: clampObjectPosition(object.x, mapData.tilewidth, width),
        y: clampObjectPosition(object.y, mapData.tileheight, height),
      })),
    };
  }
...
function editorReducer(state: EditorState, action: EditorAction): EditorState {
  switch (action.type) {
    ...
    case 'RESIZE_MAP': {
      if (!state.mapData) return state;
      const width = Math.max(1, action.width);
      const height = Math.max(1, action.height);
      if (state.mapData.width === width && state.mapData.height === height) return state;

      const resizedLayers = state.mapData.layers.map((layer) => resizeLayer(layer, state.mapData!, width, height));

...
import { Undo2, Redo2, HelpCircle, ChevronDown, Paintbrush, Eraser, MousePointer2, Move, Globe, Save, ArrowLeft } from 'lucide-react';
    ...
    <div className="flex items-center h-10 bg-surface border-b border-border px-1 select-none flex-shrink-0">
      ...
      <ToolGroup>
        <Dropdown label={t('mapEditor.toolbar.file')}>
          ...
          {onSaveAndGoBack && (
            <DropdownItem onClick={onSaveAndGoBack}>{t('mapEditor.toolbar.saveAndBack')}</DropdownItem>
          )}
          {!isChannelMode && (
            <DropdownItem onClick={() => { onSaveAs?.(); }}>{t('mapEditor.project.saveAs')}</DropdownItem>
          )}
          {onResizeMap && (
            <DropdownItem onClick={onResizeMap}>{t('mapEditor.toolbar.resizeMap')}</DropdownItem>
          )}
          <DropdownSeparator />

...
export default function PhaserGame({
  ...
  useLayoutEffect(() => {
    ...
    import("@/game/main").then(({ createGame }) => {
      ...
      // Trigger Phaser to re-measure its container whenever the div is resized
      // (e.g. when the NPC dock appears and the container right edge shifts)
      if (typeof ResizeObserver !== "undefined" && containerRef.current) {
        const ro = new ResizeObserver(() => {
          gameRef.current?.scale.refresh();
        });
        ro.observe(containerRef.current);
        // Store on the element so cleanup can disconnect it
      ...
      onCompositeRemote = async (data: {
        ...
        try {
          ...
          console.error("composite-remote-player failed:", data.id, err);
        }
      };
    ...
    return () => {
      // Disconnect ResizeObserver before destroying the game
      if (containerRef.current) {
        const el = containerRef.current as HTMLDivElement & { _ro?: ResizeObserver };
        el._ro?.disconnect();
      }
      if (gameRef.current) {
  ...
  // When the container CSS class changes (e.g. dock appears / disappears),
  // give the browser one frame to apply the new layout and then tell Phaser
  // to re-measure its parent so the canvas fills the updated bounds.
  useEffect(() => {
  ...
  return (
    <div
      id="game-container"
      ref={containerRef}
      className={containerClassName ?? "fixed inset-0 z-0"}
    />
  );
}

...
import NpcPowersFields, { DEFAULT_NPC_POWERS, type NpcPowersState } from "@/components/NpcPowersFields";
...
interface NpcHireModalProps {
  ...
  onPlaceOnMap: (npcData: {
    ...
    soul?: string;
    locale?: string;
    powers?: NpcPowersState;
    runtimeProvider?: "channel-default" | "openclaw" | "claude-code" | "codex";
    model?: string;
  }) => void;
  onSaveEdit?: (
    npcId: string,
    updates: { presetId?: string; name?: string; persona?: string; appearance?: unknown; direction?: string; identity?: string; soul?: string; agentId?: string; agentAction?: "select" | "create"; locale?: string; powers?: NpcPowersState; runtimeProvider?: "channel-default" | "openclaw" | "claude-code" | "codex"; model?: string },
  ) => void;
  editingNpc?: {
    id: string;
    name: string;
...
export default function NpcHireModal({
  ...
  const [powers, setPowers] = useState<NpcPowersState>(DEFAULT_NPC_POWERS);
  const [runtimeProvider, setRuntimeProvider] = useState<"channel-default" | "openclaw" | "claude-code" | "codex">("channel-default");
  ...
  const effectiveRuntimeProvider = runtimeProvider === "channel-default" ? gatewayProvider : runtimeProvider;
  const isLocalCliProvider = effectiveRuntimeProvider === "claude-code" || effectiveRuntimeProvider === "codex";
  const hasRuntimeAccess = hasGateway || isLocalCliProvider;
  const isExistingAgentSelected = hasRuntimeAccess && selectedAgentId && !createNewAgent;
  const personaCompat = identity.trim();
  const effectiveRuntimeLabel = effectiveRuntimeProvider === "codex"
    ? "Codex"
    : effectiveRuntimeProvider === "claude-code"
      ? "Claude Code"
      : effectiveRuntimeProvider === "openclaw"
        ? "OpenClaw"
        : "Padrão do canal";
  const savedRuntimeLabel = editingNpc?.runtimeProvider === "codex"
    ? "Codex"
    : editingNpc?.runtimeProvider === "claude-code"
      ? "Claude Code"
      : editingNpc?.runtimeProvider === "openclaw"
        ? "OpenClaw"
        : "Padrão do canal";
  const canSubmit =
    hasRuntimeAccess &&
  ...
  const applyPresetSelection = useCallback((presetId: string) => {
    ...
    if (hasRuntimeAccess && (!isEdit || createNewAgent || !selectedAgentId)) {
      if (!isEdit) {
  ...
  }, [createNewAgent, findPreset, hasRuntimeAccess, isEdit, locale, name, selectedAgentId, t, validateNewAgentId]);
  ...
  useEffect(() => {
    ...
    startTransition(() => {
      ...
      if (editingNpc) {
        ...
        setIdentity(editingNpc.identity || editingNpc.persona || "");
        setSoul(editingNpc.soul || "");
        if (editingNpc.powers) {
          const p = editingNpc.powers as Partial<NpcPowersState> & { envVars?: unknown };
          const envVarsArr: { key: string; value: string }[] = Array.isArray(p.envVars)
            ? (p.envVars as { key: string; value: string }[])
            : p.envVars && typeof p.envVars === "object"
              ? Object.entries(p.envVars as Record<string, string>).map(([key, value]) => ({ key, value: String(value ?? "") }))
              : [];
          setPowers({
            allowedTools: Array.isArray(p.allowedTools) ? p.allowedTools : DEFAULT_NPC_POWERS.allowedTools,
            envVars: envVarsArr,
            mcpServers: Array.isArray(p.mcpServers) ? p.mcpServers : [],
            maxTurns: typeof p.maxTurns === "number" ? p.maxTurns : DEFAULT_NPC_POWERS.maxTurns,
            timeoutMs: typeof p.timeoutMs === "number" ? p.timeoutMs : DEFAULT_NPC_POWERS.timeoutMs,
          });
        }
        setShowAdvanced(!!(editingNpc.soul || editingNpc.passPolicy));
        setShowPowers(!!editingNpc.powers);
        setRuntimeProvider(editingNpc.runtimeProvider || "channel-default");
        setRuntimeModel(editingNpc.model || "");
        ...
        if (editingNpc.agentId) {
          ...
          setSelectedAgentId(null);
          setCreateNewAgent(hasGateway || gatewayProvider === "claude-code" || gatewayProvider === "codex");
        }
      } else {
        setName("");
        setIdentity("");
        ...
        setRuntimeProvider("channel-default");
        setRuntimeModel("");
        setBodyType("male");
  ...
  }, [isOpen, editingNpc, hasGateway, isLocalCliProvider, setBodyType, setLayers, setActiveCategory]);
  ...
  const loadGatewayAgents = useCallback(async () => {
    ...
    try {
      ...
      params.set("provider", effectiveRuntimeProvider);
      if (runtimeModel.trim()) params.set("model", runtimeModel.trim());
      const res = await fetch(`/api/channels/${channelId}/gateway/agents?${params.toString()}`);
  ...
  }, [channelId, effectiveRuntimeProvider, hasRuntimeAccess, runtimeModel, t]);

  useEffect(() => {
    if (!isOpen || !hasRuntimeAccess) return;
    void loadGatewayAgents();
  }, [isOpen, hasRuntimeAccess, effectiveRuntimeProvider, loadGatewayAgents]);
  ...
  const handlePersonaPresetChange = useCallback((presetId: string) => {
    ...
    if (presetId === "custom") {
      setIdentity(`# IDENTITY.md - Quem sou eu?

- **Nome:** {{NPC_NAME}}
- **Papel:** [Descreva a função aqui]
- **Vibe:** [Personalidade em uma frase]
- **Emoji:** 🎯
...
- [O que mais valoriza no trabalho]

## Como falo

- [Tom e estilo de comunicação]
- "[Exemplo de frase típica 1]"
- "[Exemplo de frase típica 2]"
...
- Sempre falar como {{NPC_NAME}}, nunca revelar que é IA
- Responder sempre em Português
- Se não souber algo, dizer honestamente em vez de inventar`);
      setSoul(`# SOUL.md - Valores e Forma de Trabalhar

## Valores fundamentais

**[Valor 1].** [Explicação em uma frase de por que esse valor guia as decisões.]

**[Valor 2].** [Explicação em uma frase.]

**[Valor 3].** [Explicação em uma frase.]

## Como trabalho

- [Processo de trabalho passo a passo]
- [Como toma decisões]
- [Como lida com prazos e prioridades]

## Conflitos

- Quando [situação difícil 1]: "[Como reage]"
- Quando [situação difícil 2]: "[Como reage]"

## KPIs que me movem

- [Métrica ou resultado que define sucesso 1]
- [Métrica ou resultado que define sucesso 2]`);
      setIdentityCustomized(false);
      setSoulCustomized(false);
      return;
    }
  ...
  const handleCreateAgent = async () => {
    if (!hasRuntimeAccess || !createNewAgent || !newAgentId.trim()) {
      handleSubmit();
      return;
    }
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="flex-1 p-6 overflow-y-auto flex flex-col gap-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-1">Runtime do NPC</label>
                <select
                  value={runtimeProvider}
                  onChange={(e) => {
                    setRuntimeProvider(e.target.value as "channel-default" | "openclaw" | "claude-code" | "codex");
                    setSelectedAgentId(null);
                    setCreateNewAgent(true);
                    setGatewayAgents([]);
                    setGatewayConnectionState({ status: "idle" });
                  ...
                  className="w-full px-3 py-2 rounded bg-gray-800 border border-gray-700 text-white text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
                >
                  <option value="channel-default">Padrão do canal ({gatewayProvider || "sem gateway"})</option>
                  <option value="claude-code">Claude Code</option>
                  <option value="codex">Codex</option>
                </select>
                <p className="text-xs text-gray-400 mt-1">
                  {`Selecionado agora: ${effectiveRuntimeLabel}${runtimeModel.trim() ? ` · ${runtimeModel.trim()}` : " · modelo padrão"}`}
                </p>
                {isEdit && (
                  <p className="text-xs text-gray-500 mt-1">
                    {`Salvo atualmente: ${savedRuntimeLabel}${editingNpc?.model ? ` · ${editingNpc.model}` : " · modelo padrão"}`}
                  </p>
                )}
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-1">Modelo / LLM</label>
                {effectiveRuntimeProvider === "claude-code" ? (
                  <select
                    value={runtimeModel}
                  ...
                  >
                    ...
                    <option value="sonnet">Sonnet (claude-sonnet-4-5)</option>
                    <option value="haiku">Haiku (claude-haiku-4-5)</option>
                    <option value="opus">Opus (claude-opus-4-7)</option>
                  </select>
                ) : effectiveRuntimeProvider === "codex" ? (
                  <select
                    value={runtimeModel}
                    onChange={(e) => setRuntimeModel(e.target.value)}
                    className="w-full px-3 py-2 rounded bg-gray-800 border border-gray-700 text-white text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
                  >
                    <option value="">Padrão do provider (recomendado)</option>
                ...
                {effectiveRuntimeProvider === "codex" && (
                  <p className="text-xs text-amber-300 mt-1">
                    Deixe no modelo padrão do provider quando estiver usando login do Codex via conta ChatGPT.
                  </p>
                )}
              </div>
            </div>
            ...
            {/* AI Agent Section */}
            <AgentSection
              hasGateway={hasRuntimeAccess}
              gatewayProvider={effectiveRuntimeProvider}
              isEdit={isEdit}
            ...
            {/* Persona Section — em modo edit liberamos textareas mesmo com agente selecionado */}
            <PersonaSection
              isExistingAgentSelected={!!isExistingAgentSelected && !isEdit}
            ...
            <div className="border-t border-gray-800 pt-4">
              ...
              >
                <span>Powers, MCPs & Tokens</span>
                <span className="text-xs text-gray-500">{showPowers ? "▲" : "▼"}</span>
              </button>
              {showPowers && (
                <div className="mt-3">
                  <NpcPowersFields value={powers} onChange={setPowers} />
                </div>
              )}
            </div>
          </div>
...
function AgentSection({
  ...
  const isLocalCliProvider = gatewayProvider === "claude-code" || gatewayProvider === "codex";
  const providerLabel = gatewayProvider === "codex" ? "Codex" : "Claude Code";
    ...
    <div>
      ...
      {!hasGateway ? (
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2 rounded border border-green-500/30 bg-green-500/10 px-3 py-2 text-sm text-green-300">
            <span className="w-2 h-2 rounded-full bg-green-400 inline-block shrink-0" />
            <span>{providerLabel} disponível neste computador</span>
          </div>
          <div className="rounded border border-indigo-500/20 bg-indigo-500/5 px-3 py-2 text-xs text-indigo-300">
            {`Este NPC será orquestrado pelo ${providerLabel}. A disponibilidade local não garante que todo modelo selecionado seja suportado.`}
          </div>
          {agentsLoading ? (
              ...
              {createNewAgent && (
                <div>
                  <input
                    ...
                    placeholder="ex: npc-meta-ads-01"
                    className="w-full px-3 py-2 rounded bg-gray-800 border border-gray-700 text-white text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
                  />
                  {newAgentIdError && <p className="text-xs text-red-400 mt-1">{newAgentIdError}</p>}
                  <p className="text-xs text-gray-500 mt-1">{`ID único para este agente local no ${providerLabel}`}</p>
                </div>
              )}
            </>
          )}
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          if (gatewayProvider) params.set("provider", gatewayProvider);
                          const res = await fetch(`/api/channels/${channelId}/gateway/agents?${params.toString()}`, { method: "DELETE" });

...
app.prepare().then(async () => {
  ...
  registerRpcHandler(async (channelId, method, params) => {
    const gateway = await getOrConnectGateway(channelId, {
      provider: typeof params.runtimeProvider === "string" ? params.runtimeProvider : undefined,
      workspacePath: typeof params.workspacePath === "string" ? params.workspacePath : undefined,
      model: typeof params.runtimeModel === "string" ? params.runtimeModel : undefined,
    });
  ...
  httpServer.listen(port, () => {
    ...
    if (process.env.GROQ_API_KEY) {
      console.log(`> Groq fallback armed — modelo ${process.env.GROQ_MODEL || "llama-3.3-70b-versatile"} (ativa se Claude/Codex esgotarem tokens)`);
    } else {
      console.log(`> Groq fallback não configurado (defina GROQ_API_KEY para ativar)`);
    }
  });
});

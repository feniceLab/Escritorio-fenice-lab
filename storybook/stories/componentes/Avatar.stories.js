export default {
  title: 'Starkën Design System/Componentes/Avatar & Favicon',
  tags: ['autodocs'],
  parameters: {
    docs: {
      description: {
        component: 'Avatares circulares usando o logo oficial Starkën. Também documenta o uso como favicon em abas, PWAs e atalhos.',
      },
    },
  },
};

const FAVICON_SRC = './logo-fenix-tecnologia-square.png';
    ...
    <div style="padding: 2rem; font-family: 'Inter', sans-serif;">
      ...
      <div style="background: #0f172a; border-radius: 16px; padding: 2rem; margin-bottom: 2rem; border: 1px solid rgba(255,255,255,0.08);">
        ...
        <div style="display: flex; align-items: flex-end; gap: 2rem; flex-wrap: wrap;">
          ${[
            { label: 'XS',  size: 16,  use: 'Favicon tab' },
            { label: 'SM',  size: 24,  use: 'Inline icon' },
            { label: 'MD',  size: 32,  use: 'Navbar' },
            { label: 'LG',  size: 48,  use: 'Card avatar' },
            { label: 'XL',  size: 64,  use: 'Profile' },
            { label: '2XL', size: 96,  use: 'Hero' },
            { label: '3XL', size: 144, use: 'Splash' },
          ].map(({ label, size, use }) => `
    ...
    <div style="padding: 2rem; font-family: 'Inter', sans-serif;">
      ...
      <div style="display: flex; gap: 2.5rem; flex-wrap: wrap; align-items: center;">
        ${[
          ...
          { status: 'busy',    color: '#f59e0b', label: 'Ocupado' },
          { status: 'offline', color: '#64748b', label: 'Offline' },
          { status: 'dnd',     color: '#ef4444', label: 'Não Perturbar' },
        ].map(({ color, label }) => `

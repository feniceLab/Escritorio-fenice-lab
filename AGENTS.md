# Fenix Lab - Project Context

## Overview
**Fenix Lab** is a unified SPA built in a single HTML file (`checklist-relatorios.html`) with no framework.
- **Repo**: `ferramentastecnologia/fenix-lab`
- **Live**: `https://escritorio.starkentecnologia.com.br/fenix-lab/`
- **Source of truth**: VPS path `/var/www/fenix-lab`
- **Branch**: local VPS Git history

## Mandatory VPS Workflow
This project is operated from the VPS. When working on the live office app, agents must treat `/var/www/fenix-lab` on `root@187.77.46.199` as the primary source.
...
- SSH into `root@187.77.46.199` and work in `/var/www/fenix-lab`.
...
- Create a safety point with `fenix-lab-backup` or `fenix-lab-checkpoint "message"` when the change touches live behavior.
...
- Use `fenix-lab-checkpoint "message"` for a safe backup plus commit once the change is validated.

Preferred live files:
- Main SPA: `/var/www/fenix-lab/checklist-relatorios.html`
- Mirror entry: `/var/www/fenix-lab/index.html`
- Content API: `/var/www/fenix-lab/api/content.js`
- Meta publishing API: `/var/www/fenix-lab/api/meta/publish.js`
